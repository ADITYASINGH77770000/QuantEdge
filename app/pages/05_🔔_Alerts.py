...
# Alert engine config overrides
with st.sidebar.expander("⚙️ Thresholds", expanded=False):
    sig_thresh   = st.slider("Signal",      0.50, 1.00, 0.80, 0.05)
    bear_thresh  = st.slider("Bear Prob",   0.50, 0.95, 0.70, 0.05)
    dd_mult      = st.slider("Drawdown ×",  1.0,  4.0,  2.0,  0.5)
    var_mult     = st.slider("VaR ×",       1.0,  3.0,  1.5,  0.5)
    sharpe_min   = st.slider("Min Sharpe", -1.0,  3.0,  1.0,  0.25)
    vol_mult     = st.slider("Volume ×",    1.5,  5.0,  2.5,  0.5)

with st.sidebar.expander("🔕 Cooldown (sec)", expanded=False):
    cd_risk      = st.number_input("Risk & VaR",    value=14400, step=3600)
    cd_regime    = st.number_input("Regime & EW",   value=7200,  step=1800)
    cd_signal    = st.number_input("Signal & Pred", value=3600,  step=900)
    cd_anomaly   = st.number_input("Anomaly",       value=1800,  step=600)
